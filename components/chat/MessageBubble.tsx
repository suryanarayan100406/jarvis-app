...
import { Smile, ThumbsUp, Heart, Flame, Zap } from 'lucide-react'
...
export function MessageBubble({ isOwn, content, timestamp, senderName }: MessageProps) {
    const [showReactions, setShowReactions] = useState(false)
    const [reactions, setReactions] = useState<string[]>([])

    const addReaction = (label: string) => {
        setReactions(prev => [...prev, label]) // Simple local state for demo
        ...
        >
            <div className="relative group max-w-[80%] md:max-w-[60%]">
                ...
                <motion.div
                    ...
                    // Long press simulation for mobile could be added here
                    className={cn(
                        ...
                        <div className="absolute -bottom-3 right-0 flex -space-x-1">
                            {reactions.slice(0, 3).map((r, i) => (
                                <div key={i} className="bg-zinc-800 rounded-full p-1 border border-black text-[10px]">
                                    {/* Map label back to icon? For MVP just showing logic */}
                                    ❤️
                                </div>
                            ))}
                        </div>
                ...
                {/* Reaction Picker (Telegram Style Floating) */}
                <AnimatePresence>
                    {(showReactions || false) && ( // Simplified: using hover group makes it easier normally but let's use state for 'click/longpress' logic
                        <motion.div
            ...
            {/* Trigger for interaction hints on desktop */}
            <div
                className="opacity-0 group-hover:opacity-100 transition-opacity self-center px-2 cursor-pointer"
                onClick={() => setShowReactions(!showReactions)}
            >
                <Smile className="w-4 h-4 text-muted-foreground hover:text-white" />
            </div>
        </div>
    )
}
